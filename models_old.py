...
class DatabaseManager:
    ...
    def get_all_bases_calculo(self):
        ...
        if conn is not None:
            try:
                cursor = conn.cursor()
                cursor.execute("SELECT descricao FROM tb_base_calculo ORDER BY id")
    ...
    def populate_default_data(self):
        ...
        if conn is not None:
            try:
                ...
                c.executemany(
                    "INSERT OR IGNORE INTO tb_base_de_calculo (descricao) VALUES (?)",
                    [(base,) for base in bases_de_calculo]
                )
        ...
        if conn is not None:
            try:
                ...
                valores = (
                    ...
                    dados.get("Fora do País", "Não"),  # Novo campo
                    dados["CNPJ"],
                    fornecedor_id,
                    dados["Inscrição Municipal"],
                ...
                print(f"Erro ao inserir nota fiscal: {str(e)}")
                conn.rollback()
                raise e
            finally:
                conn.close()
    ...
    def update_nota_fiscal(self, id_nota, dados):
        ...
        if conn is not None:
            try:
                ...
                # [código de busca de IDs permanece igual...]
                ...
                cursor.execute(
                    """
                        ...
                        """,
                     (
                    dados["referencia"],  # Nome corrigido
                    dados["cadastrado_goiania"],  # Nome corrigido
            ...
            try:
                ...
                cursor.execute(
                    ...
                    (
                        dados["Referencia"],
                        dados["cadastrado_goiania"],
                        dados.get("fora_pais", "Não"),  # Novo campo
                        dados["CNPJ"],
                        fornecedor_id,
                        dados["Inscrição Municipal"],
                        dados["Tipo de Serviço"],
